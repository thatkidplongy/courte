...
const SeriesSummary = ({ summary }: { summary: NonNullable<CreateSeriesFormState['summary']> }) => (
  <div className="rounded-xl bg-court-50 px-5 py-4 text-sm">
    <p className="font-semibold text-court-800">
      Booked {summary.created} of {summary.requested} weeks 🎉
    </p>
    {summary.conflicts.length > 0 ? (
      <p className="mt-1 text-court-800/80">Already taken: {summary.conflicts.join(' · ')}</p>
    ) : null}
    <Link href="/bookings" className="mt-2 inline-block font-semibold text-court-700 underline underline-offset-2">
      View my bookings →
    </Link>
  </div>
);

export const BookingForm = ({ courtId, slots, durations, preselectedStartIso, action, seriesAction }: BookingFormProps) => {
