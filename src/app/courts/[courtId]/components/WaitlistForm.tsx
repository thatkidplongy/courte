...
export const WaitlistForm = ({ courtId, hourOptions, action }: WaitlistFormProps) => {
  ...
  if (state.ok) {
    return (
      <p className="rounded-xl bg-court-50 px-5 py-4 text-sm font-medium text-court-800">
  ...
  return (
    <form action={formAction} className="flex flex-col gap-4">
      ...
      {state.error ? <p className="rounded-xl bg-red-50 px-4 py-2.5 text-sm font-medium text-red-700">{state.error}</p> : null}
    </form>
  );
};
