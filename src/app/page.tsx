...
import { DEFAULT_SEARCH_RADIUS_METRES, SEARCH_DEFAULTS, SLOT_CHIPS_PER_CARD, SPORTS, type Sport } from '@/consts';
import { BoltIcon, CheckIcon, ClockIcon, PinIcon, ShieldIcon } from '@/components/icons';
import { searchCourtsByProximity, type CourtSearchResult } from '@/db/repositories/courtRepository';
...
const SPORT_BANNERS: Record<Sport, string> = {
...
const SearchBar = ({ sport, dateIso }: SearchInput) => (
  ...
  >
    <label className="flex flex-1 flex-col gap-1 sm:px-4 sm:py-1.5">
      ...
      <select name="sport" defaultValue={sport} className="bg-transparent text-sm font-semibold text-neutral-900 focus:outline-none">
        {SPORTS.map(option => (
    ...
    <label className="flex flex-1 flex-col gap-1 sm:px-4 sm:py-1.5">
      ...
      <span className="flex items-center gap-1.5 text-sm font-semibold text-neutral-900">
        <PinIcon className="h-3.5 w-3.5 text-court-600" />
        Quezon City
      </span>
    </label>
    <label className="flex flex-1 flex-col gap-1 sm:px-4 sm:py-1.5">
  ...
  <ul className="flex flex-wrap items-center gap-x-7 gap-y-2">
    {TRUST_BADGES.map(badge => (
      <li key={badge.label} className="flex items-center gap-2 text-sm text-neutral-400">
        <badge.icon className="h-4 w-4 text-court-400" />
        {badge.label}
      </li>
    ))}
  </ul>
...
const CourtCard = ({ court, dateIso, slotStarts, ratePerHourCents }: CourtCardProps) => (
    ...
    <div className="p-5">
      ...
      {ratePerHourCents !== null ? (
        <p className="mt-2 text-sm text-neutral-500">
          from <span className="text-base font-bold text-court-700">₱{Math.round(ratePerHourCents / 100)}</span>
          /hr
        </p>
      ) : null}
      <div className="mt-4 flex flex-wrap items-center gap-2">
        {slotStarts.length === 0 ? (
          ...
          slotStarts.map(start => (
            <Link key={start} href={`/courts/${court.id}?date=${dateIso}&start=${start}`} className="chip">
              {formatTime(new Date(start), court.venueTimezone)}
            </Link>
          ))
        )}
        <Link
          href={`/courts/${court.id}?date=${dateIso}`}
          className="ml-auto text-sm font-medium text-court-700 transition group-hover:translate-x-0.5 hover:text-court-800"
        >
          all times →
        </Link>
      </div>
...
const EmptyState = ({ sport }: { sport: Sport }) => (
  <div className="card p-12 text-center">
    <p className="text-lg font-medium text-neutral-700">No {SPORT_LABELS[sport].toLowerCase()} courts nearby that day</p>
...
const StatsBand = () => (
  <section className="mt-16 rounded-3xl bg-court-50 px-8 py-10">
    <dl className="grid grid-cols-2 gap-8 sm:grid-cols-4">
      {STATS.map(stat => (
        <div key={stat.label} className="text-center">
          <dd className="text-3xl font-extrabold tracking-tight text-court-800">{stat.value}</dd>
...
const SearchPage = async ({ searchParams }: PageProps) => {
  ...
  const day = DateTime.fromISO(dateIso, { zone: SEARCH_DEFAULTS.timezone }).startOf('day');
  const range = { start: day.toMillis(), end: day.plus({ days: 1 }).toMillis() };

  const { courts } = await searchCourtsByProximity({
    sport,
    longitude: SEARCH_DEFAULTS.longitude,
    latitude: SEARCH_DEFAULTS.latitude,
    radiusMetres: DEFAULT_SEARCH_RADIUS_METRES,
  });
  ...
  const baseRateFor = (courtId: string): number | null => {
    const rates = priceRules.filter(rule => rule.courtId === courtId && !rule.memberOnly);
          ...
          <ul className="mt-6 grid gap-6 sm:grid-cols-2">
            {courts.map(court => (
              <CourtCard
                ...
                court={court}
                dateIso={dateIso}
                ratePerHourCents={baseRateFor(court.id)}
                slotStarts={listStartTimes({
