...
import type { BookingStatus } from '@/consts';
...
const BookingRow = ({ booking, venueId, timezone }: { booking: VenueBooking; venueId: string; timezone: string }) => {
    ...
    <li className="flex flex-wrap items-center justify-between gap-4 border-t border-neutral-100 py-4 first:border-t-0">
      <div>
        ...
        <p className="mt-1 text-sm text-neutral-500">
          {booking.courtName} · {formatTime(booking.playStart, timezone)}–{formatTime(booking.playEnd, timezone)} ·{' '}
      ...
      {outstandingCents > 0 ? (
        ...
        <span className="rounded-full bg-court-100 px-3 py-1 text-xs font-semibold text-court-800">settled</span>
      )}
    </li>
  );
};
...
const ManageVenuePage = async ({ params }: PageProps) => {
  ...
  const timezone = (await findVenueTimezones([venueId])).get(venueId);
  ...
  const [courts, bookings, stats] = await Promise.all([
    findCourtsByVenue(venueId),
    findVenueBookings(venueId, now.toJSDate(), now.plus({ hours: 24 }).toJSDate()),
    getVenueStats(
      venueId,
      dayStart.toJSDate(),
      dayEnd.toJSDate(),
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-12">
      <p className="text-sm font-medium text-court-700">For venue owners</p>
