...
describe('mergeIntervals', () => {
  ...
  it('sorts unordered input', () => {
    expect(mergeIntervals([{ start: 50, end: 60 }, { start: 10, end: 20 }])).toEqual([
      { start: 10, end: 20 },
      { start: 50, end: 60 },
    ]);
  });
...
describe('subtractIntervals', () => {
  it('punches holes and keeps the remainder', () => {
    const remaining = subtractIntervals([{ start: 0, end: 100 }], [{ start: 20, end: 30 }, { start: 60, end: 70 }]);
  ...
  it('clips holes that extend past either edge', () => {
    expect(subtractIntervals([{ start: 10, end: 20 }], [{ start: 0, end: 12 }, { start: 18, end: 30 }])).toEqual([
      { start: 12, end: 18 },
    ]);
  });
});
...
describe('doesSlotFit', () => {
  ...
  it('accepts a slot fully inside a free interval', () => {
    expect(
      doesSlotFit({ availability, requested: { start: at(`${MONDAY}T11:00`), end: at(`${MONDAY}T12:30`) } })
    ).toBe(true);
  });

  it('rejects a slot that leaks past the free interval edge', () => {
    expect(
      doesSlotFit({ availability, requested: { start: at(`${MONDAY}T13:00`), end: at(`${MONDAY}T14:30`) } })
    ).toBe(false);
  });
  ...
  it('rejects a slot spanning two free intervals with a gap between', () => {
    ...
    expect(doesSlotFit({ availability: split, requested: { start: at(`${MONDAY}T11:00`), end: at(`${MONDAY}T14:00`) } })).toBe(
      false
    );
  });
});
