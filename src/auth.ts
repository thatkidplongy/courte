 ...
 *
 * Deliberately no database adapter. Google is an identity source, not our user store: on
 * sign-in we upsert into our own users table and put OUR uuid in the token. The session
 * carries identity only — venue memberships are looked up fresh per request
 * (src/domain/authz), so revoking staff access never waits for a token to expire.
 */
...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  callbacks: {
    ...
    async jwt({ token, account, profile, user }) {
      ...
      if (account && email) {
        token.userId = await upsertUserFromOAuth({
          email,
          name: (account.provider === 'google' ? profile?.name : user?.name) ?? email,
        });
      }
      return token;
    },
