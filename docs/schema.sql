...
-- Anchored to a booking, never to a venue: booking_id UNIQUE means only somebody who booked can
-- write one, and exactly one per booking. The service adds that play must have finished and the
-- booking must belong to the caller.
CREATE TABLE "Review" (
  id         bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  booking_id bigint NOT NULL UNIQUE REFERENCES "Booking"(id) ON DELETE CASCADE,
  venue_id   bigint NOT NULL REFERENCES "Venue"(id) ON DELETE CASCADE,   -- denormalised: every read groups by it
  user_id    bigint NOT NULL REFERENCES "User"(id)  ON DELETE CASCADE,
  rating     smallint NOT NULL CHECK (rating BETWEEN 1 AND 5),
  body       text CHECK (body IS NULL OR length(btrim(body)) > 0),       -- optional, but never blank
  deleted_at timestamptz,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
-- A view, like "BookingPaymentState": a computed aggregate cannot drift from its rows. LEFT
-- JOIN so every venue appears — an unrated one gets count 0 and a NULL average, not zero, which
-- would sort it below a genuinely bad venue. "Top rated" is derived on read from the contract's
-- thresholds and is deliberately not a column here.
