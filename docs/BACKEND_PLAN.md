...
`?amenities=parking,showers`, and a uuid would mean a lookup on every filter parse.
...
CREATE INDEX reviews_venue_idx ON reviews (venue_id, created_at DESC);

CREATE VIEW venue_rating AS
...
CREATE INDEX venue_passes_lookup ON venue_passes (venue_id, user_id);
```
...
```sql
ALTER TABLE price_rules ADD CONSTRAINT price_rules_window_forward
...
- `getVenueRevenueByDay(venueId, from, to, timezone)` — `generate_series` LEFT JOIN payments,
