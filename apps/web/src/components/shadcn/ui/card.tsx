...
function CardTitle({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn('font-heading text-base font-medium leading-snug group-data-[size=sm]/card:text-sm', className)}
      {...props}
    />
  );
}
...
function CardFooter({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn('bg-muted/50 p-(--card-spacing) flex items-center rounded-b-xl border-t', className)}
      {...props}
    />
  );
}
